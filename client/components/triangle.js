...
export class Triangle extends React.Component {
  ...
  addCoordinatesToData() {
    ...
    const descriptions = {
      ...
      Acousticness:
        "An average across all your tracks of Spotify's confidence measure of whether the track is acoustic",
      Valence:
        'A measure describing the musical positiveness: high valence sound more happy, tracks with low valence sound more sad',
      Danceability:
        'Describes how danceable a track is based on tempo, rhythm stability, beat strength, and overall regularity',
      Energy:
        'Represents a measure of intensity and activity—energetic tracks feel fast, loud, and noisy',
    ...
    for (let i = 0; i < xCoords.length; i++) {
      ...
      this.props.features.data[i].description =
        descriptions[this.props.features.data[i].Name]
    }
  ...
  createTriangle() {
    ...
    const node = svg
      ...
      .attr('transform', function(d) {
        return (
          'translate(' +
          (20 + Number(d.data.xPosition)) +
          `,` +
          (20 + Number(d.data.yPosition)) +
          `)`
        )
      })
    ...
    linearGradient
      ...
      .duration(dur)
      .attr('offset', function(d) {
        return `${d.data.Count * 0.2}%`
      })
      .attr('offset', function(d) {
        return `${d.data.Count * 0.4}%`
      })
      .attr('offset', function(d) {
        return `${d.data.Count * 0.6}%`
      })
      .attr('offset', function(d) {
        return `${d.data.Count * 0.8}%`
      })
      .attr('offset', function(d) {
        return `${d.data.Count}%`
      })
    ...
    node
      ...
      .duration(dur2)
      .text(function(d) {
        return `${Math.floor(d.data.Count * 0.2)} %`
      })
      .transition()
      .duration(dur2)
      .text(function(d) {
        return `${Math.floor(d.data.Count * 0.4)} %`
      })
      .transition()
      .duration(dur2)
      .text(function(d) {
        return `${Math.floor(d.data.Count * 0.6)} %`
      })
      .transition()
      .duration(dur2)
      .text(function(d) {
        return `${Math.floor(d.data.Count * 0.8)} %`
      })
      .transition()
      .duration(dur2)
      .text(function(d) {
